...
const Posts = ({ selectedCategory }) => {
    ...
    //     try {
    //         const response = await getPosts(filteredPosts.length)
    //         const postsData = response.data;
    //         setHasMorePosts(postsData.length > 0)
    //         let postData = [...filteredPosts, ...postsData]
    //         // setPosts(postsData);

    //         // Check if the current user has liked each post
    //         const likedPosts = postData.map((post) => ({
    //             ...post,
    ...
    //     try {
    //         const response = await getLandingPosts(filteredPosts.length)
    //         const postsData = response.data;
    ...
    try {
        ...
        const uniquePosts = Array.from(new Set([...filteredPosts, ...likedPosts]));
...
const fetchLandingPosts = async () => {
    try {
        ...
        const uniquePosts = Array.from(new Set([...filteredPosts, ...postsData]));
    ...
    return (
        <InfiniteScroll
            ...
            next={ userInfo ? fetchPosts : fetchLandingPosts }
            hasMore={hasMorePosts}
            loader={filteredPosts.length> 0 ? <Loader /> : ""}
            endMessage={
